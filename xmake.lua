package('glfw3')
    set_urls('https://github.com/glfw/glfw/releases/download/$(version)/glfw-$(version).zip')
    add_versions('3.3', '36fda4cb173e3eb2928c976b0e9b5014e2e5d12b9b787efa0aa29ffc41c37c4a')
    add_deps('cmake')
    on_install(function (package)
        import('package.tools.cmake').install(package)
    end)
package_end()

add_requires('glfw3', {optional = true})
add_requires('glew', {optional = true})

rule('bms')
    set_extensions('.bms', '.bme', '.bml', '.pms')
    on_build_file(function (target, source)
        os.cp(source, path.join(target:targetdir(), source))
    end)

target('flattest')
    set_kind('binary')
    add_rules('bms')
    add_includedirs('.')
    add_headerfiles('bmflat.h')
    add_files('bmflat.c')
    add_files('examples/flattest.c')
    add_files('examples/sample.bms')

target('flatspin')
    set_kind('binary')
    add_packages('glfw3')
    add_packages('glew')
    if is_plat('macosx') then
        add_frameworks('OpenGL', 'Cocoa', 'IOKit', 'CoreVideo')
    elseif is_plat('linux') then
        add_links('m', 'dl', 'pthread', 'GL')
    elseif is_plat('windows') then
        add_links('user32', 'ole32', 'comdlg32', 'shell32', 'gdi32', 'opengl32')
    end
    if is_mode('release') then
        set_optimize('faster')
    end
    add_includedirs('.')
    add_includedirs('examples')
    add_headerfiles('bmflat.h')
    add_files('bmflat.c')
    add_files('examples/flatspin.c')
    add_files('examples/miniaudio/extras/stb_vorbis.c')
    add_files('examples/tinyfiledialogs/tinyfiledialogs.c')
